<template>
    <div id="wplayer" @mouseenter="enter" @mouseleave="leave">
        <div ref="playerRef" class="wplayer-container wplayer-uncheck" @contextmenu.prevent="openCtxMenu"
            ...
            <video class="player" ref="videoRef" @canplay="initVideo" @timeupdate="timeUpdate" @ended="videoEnd"
                @waiting="videoWaiting" @playing="videoPlaying" autoplay muted></video>
            <!-- 控制栏 -->
            <div class="control" v-show="showControl">
                <player-control ref="controlRef" :mobile="playerOptions?.mobile" :buffering="videoBuffering"
                    ...
                    @progress-change="setProgress" @full="fullScreen" @quality-change="setQuality" @speed-change="setSpeed"
...
import useMsg from '../Barrage/msg';
import useConfig from '../Barrage/config';
import useQuality from '../Barrage/quality';
import useShortcutKey from '../Barrage/shortcut-key';
import useFullScreen from '../Barrage/full-screen';
import PlayerMsg from '../Barrage/player-msg.vue';
import PlayerControl from '../Barrage/player-control.vue';
import DanmakuSend from '../Barrage/danmaku-send.vue';
import ContextMenu from '../Barrage/context-menu.vue';
import PlayerBuffering from '../Barrage/player-buffering.vue';
import DanmakuContainer from '../Barrage/danmaku-container.vue';
import { ref, onMounted, onBeforeUnmount, watch, nextTick } from 'vue';

import { handleOptions } from "../Barrage/options";
import type { DanmakuType, FilterDanmakuType } from '../types/barrage';
import type { OptionsType, QualityType } from '../types/barrage';
...
const options = {
    ...
    customType: function (player: HTMLVideoElement, url: string) {
        const flv = flvjs.createPlayer({
            type: 'flv',
            url: 'http://192.168.192.131:5001/live/SupperMoment.flv',
        });
...
const handleDiffQualityResource = (quality: number, options: OptionsType) => {
    const resource = options.resource as unknown as QualityType;
...
const danmakuList = ref<Array<DanmakuType>>(options?.danmaku?.data || [])
...
const filterDanmaku = (filter: FilterDanmakuType) => {
    ...
    const originalDanmaku = options?.danmaku?.data || [];
...
// 监听传入弹幕改变
watch(() => danmakuKey, () => {
    if (options.danmaku?.data) {
        danmakuList.value = options.danmaku.data;
...
onMounted(() => {
    // 读取配置信息
    const op = handleOptions(options);
    //处理视频资源信息
    const resource = op.resource as unknown as QualityType;
    // 获取视频清晰度信息
    const initialQuality = getInitialQuality(resource);
    //处理视频播放
    handleDiffQualityResource(initialQuality, op);
    //监听快捷键
    ...
    controlRef.value?.setpPlaybackSpeed(op.playbackSpeed!);
    playerOptions.value = op;
